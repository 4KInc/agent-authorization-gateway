...
## Receipts
- Signed receipts on a contiguous, integrity-verified chain (grows with each pipeline run)
- Chain integrity: contiguous hash-linked, verify-chain returns PASS

## Audit Reports
- Audit reports generated for every receipt, with verbatim compliance citations
- Verdicts: ALIGNED, CONFLICT, INSUFFICIENT_EVIDENCE, ERROR

## Compliance Citations
- Verbatim citations from OWASP NHI Top 10, NIST SP 800-53, and NIST AI RMF
- RAG retrieval via Vertex AI Search

## Other Artifacts
- Policy proposals generated by the Recommender from pattern detection across CONFLICT audits
- Incident reports assembled by the Investigator from CONFLICT-triggered evidence
- Resources spanning all five supported types (db, api, storage, queue, function)
- Cloud Run services deployed: 16 (agent-auth-* plus demo-agent and supporting services)
- Unit tests passing: 314/314 (0 failures)
- Total MCP tools: 30 (register_agent removed for PoP enforcement)
- REST API endpoints: 38
- Lines of Python code: 23,000+
...
- Cloud Run (16 services, scale-to-zero)
